...
class ElecraftRadio(chirp_common.CloneModeRadio):
    ...
    def get_memory(self, number: int|str) -> self.ECMemory:
        ...
        if not self._memobj:
            mem.empty = True
            LOG.warning(f'get_memory: no _memobj')
            return mem
        if not self._memobj.radiomem:
            mem.empty = True
            LOG.warning(f'get_memory: no _memobj.radiomem')
            return mem
    ...
    def set_memory(self, memory: self.ECMemory) -> None:
        ...
        if not memory.extra:
            raise errors.RadioError(f'Invalid CHIRP memory: no extra in "{memory}"')
